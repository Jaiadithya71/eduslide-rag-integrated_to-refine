...
def render_main_form():
    ...
    # ------------------------------------------------
    # SYNC LOGIC FOR IMAGE CHECKBOXES
    # ------------------------------------------------
    # 1. Initialize Master State
    if "include_images" not in st.session_state:
        st.session_state.include_images = True

    # 2. Ensure Widget Keys Exist & Match Master (for first run / reload)
    if "chk_preview" not in st.session_state:
        st.session_state.chk_preview = st.session_state.include_images
    if "chk_advanced" not in st.session_state:
        st.session_state.chk_advanced = st.session_state.include_images

    # 3. Callbacks to Sync Peers
    def sync_from_preview():
        # User changed Preview -> Update Master & Advanced
        val = st.session_state.chk_preview
        st.session_state.include_images = val
        st.session_state.chk_advanced = val

    def sync_from_advanced():
        # User changed Advanced -> Update Master & Preview
        val = st.session_state.chk_advanced
        st.session_state.include_images = val
        st.session_state.chk_preview = val
    ...
    with col2:
        ...
        # LINKED CHECKBOX 1
        include_images_preview = st.checkbox(
            "Include images",
            key="chk_preview",
            on_change=sync_from_preview
        )
        time_estimate = estimate_generation_time(num_slides, st.session_state.include_images)
...
def render_results(result):
    ...
    if result.get("rag_sources"):
        ...
        for source in result["rag_sources"][:5]:
            st.markdown(f"""
            <div class="metric-card">
                <p><strong>📄 {source['source']}</strong> - Page {source['page']}</p>
                <p style="color:#666;font-size:0.9rem;">
                    Relevance: {source['score']:.1%}
                </p>
            </div>
            """, unsafe_allow_html=True)
